...
$configBefore = Get-Content -Raw -LiteralPath '.qa/worldmap-274-installed-config-before.json' | ConvertFrom-Json
...
$result | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath '.qa/worldmap-274-install-verification.json'
$result | Format-List
if ($version -ne '2.7.4' -or $rootHash -ne $installedHash -or $shortcut.TargetPath -ne $installedExe -or $shortcut.WorkingDirectory -ne $installedDir -or -not $shortcut.IconLocation.StartsWith($installedDir) -or -not $preserved -or -not $preserved2) { throw 'Installed release verification failed' }
