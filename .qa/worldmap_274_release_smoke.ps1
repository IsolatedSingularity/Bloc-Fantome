...
foreach ($exePath in @($rootExe,$installedExe)) {
 ...
 $appResponded = @($windowDetails | Where-Object {$_.Class -eq 'pygame' -and $_.Responsive}).Count -gt 0
 ...
 $results += [pscustomobject]@{Path=$exePath;Version=(Get-Item -LiteralPath $exePath).VersionInfo.FileVersion;ProcessCount=$running.Count;WindowCount=$handles.Count;ResponsiveWindows=$responsive;Windows=$windowDetails;AppResponded=$appResponded;Remaining=$remaining;SHA256=(Get-FileHash -LiteralPath $exePath).Hash}
}
$results | ConvertTo-Json -Depth 6 | Set-Content .qa/worldmap-274-release-smoke.json
$results | ConvertTo-Json -Depth 5
if (@($results | Where-Object {-not $_.AppResponded -or $_.Remaining -ne 0}).Count) {throw 'Smoke failed'}
