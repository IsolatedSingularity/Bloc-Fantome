...
$stageFolder = Join-Path $PSScriptRoot 'start-menu-290'
...
if ($version -ne '2.9.0') { throw "Unexpected installed version: $version" }
$report = [pscustomobject]@{ Version=$version; SHA256=$installedHash; Shortcuts=$results }
$report | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $PSScriptRoot 'consistency-start-menu-290.json')
