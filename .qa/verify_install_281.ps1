...
$configBefore = Get-Content -Raw -LiteralPath '.qa/guided-281-installed-config-before.json' | ConvertFrom-Json
...
$required = @('Assets\Fonts\Zekton\Zekton-Regular.otf','Assets\Icons\Splash_Midnight_Workshop.png','References\Titles\horror.png')
...
$result | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath '.qa/guided-281-install-verification.json'
$result | Format-List
if ($version -ne '2.8.1' -or $rootHash -ne $installedHash -or $shortcut.TargetPath -ne $installedExe -or $shortcut.WorkingDirectory -ne $installedDir -or -not $shortcut.IconLocation.StartsWith($installedDir) -or -not $preserved -or -not $preserved2) { throw 'Installed release verification failed' }
